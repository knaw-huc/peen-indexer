...
def store_document(elastic: Elasticsearch, index: str, doc: dict[str, any]) -> None:
    ...
    if resp["result"] == "created":
        logger.success(
            "Indexed {}: {:>3} entities", doc_id, len(doc.get("entityNames", []))
        )
    else:
...
def build_overlapping_types_query(
    item: SearchResultItem, types: list[str]
) -> dict[str, any]:
...
def fetch_overlapping_annotations(
    container: ContainerAdapter, item: SearchResultItem, types: list[str]
